...
| # | bug | your words / the rule |
|---|---|---|
| 105.1 | ➡️ **Moved to `BL-313`** (your call, 2026-09-26: the second time). The text is TMP's missing-glyph warning: *"the unicode character [] cannot be found in [LiberationSans SDF] and in any fallback fonts and was replaced with character [] in text object [label]"*. | *"I Still get the chat message flood for invalid character"* |
| 105.2 | ✅ **FIXED 0.214.12** — **Untrack did nothing.** The client's `QuestAction` refused every action but Abandon unless an NPC dialog was open, so "track" never left the phone (the same bug Abandon had in playtest 14). Being the last quest had nothing to do with it. The SmokeTest calls the hub directly, so it could not see it. The Details page's Track button (0.214.5) had the same bug and is fixed with it. | *"in the quest window where all quest are listed I have abandon/untrack/details buttons and the untack does nothing .. Its the only quest there"* |
| 105.3 | ✅ **NOT A BUG** — your character was 90 and the rune quest closes after 75 (*"I was 90 my bad"*). — **`/resetlimits` does not give back the daily Apothecary rune quest** (0.214.1, `BL-296`). The SmokeTest now plays exactly this: a level-10 character hands the rune quest in at the Apothecary, it reads "Done today", `/resetlimits`, it reads Available and the Apothecary gives it again. The recipe dailies pass the same check. | *"rerol OK but the reset limits don't reset my daily apoth rune quest"*. ❓ **Was the character below level 6 at the time?** The re-roll takes you back to level 1, and the rune quest only opens at **6** (and closes after 75), so right after a re-roll the Apothecary has nothing to offer, reset or not. If you were 6-75, tell me what you saw (no `!`, the quest window's status text), and whether her dialog was already open when you reset. |
| 105.4 | ✅ **FIXED 0.214.3** — **The crafter window's "browsing / at anvil" row was hidden behind the tabs** (0.203.0). The header wrapped out of its one 22px line; it now has three lines of its own (level·slots·points / type levels / where you are) and the tabs sit below it. | *"The row where is says "browsing/at anvil" is hidden behind the tabs"* |
