...
| 105.2 | 🟡 **0.214.5 added Track/Untrack to the Details window** (with `BL-311`); ❓ still: was that where you were? — **The quest window will not UNTRACK a quest** (0.213.0, the quest arrow). **The server side works**: the SmokeTest now accepts a quest (pinned), presses Untrack (unpinned) and Track (pinned again). The Untrack button on an **Active-tab row** sends exactly that. The **Details** window has **no** Track/Untrack button at all. | *"the quest window dont allow me to untrack it"*. ❓ **Were you in the Details window?** If so, it is missing, not broken, and it lands with `BL-311` (the `[location tracking]` button goes in the same place). If it was the row's own Untrack button, say what happened when you pressed it. |
