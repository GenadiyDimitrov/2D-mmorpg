...
## `BL-289` ✅ CLOSED 2026-09-25 in **0.209.2**: a grade only where one exists, and the temporary box names

Built as written: one `ItemCatalog.GradeLabel` behind every grade print (gear = its letter, a box = its contents' grade, the
rest "-"), and the boxes renamed "Temporary Common Weapon/Armor Selection Box". A playtest may reopen it. The entry as it stood:

### `BL-289` 🔴 (as it stood) THE GRADE ON THINGS THAT HAVE NO GRADE — and the temporary boxes' names
