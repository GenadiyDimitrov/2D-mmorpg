...
| **Weapon mastery** | ⚠️ **data-driven, NUMBERS UNTUNED (2026-06-27)** | Weapon-CONDITIONAL masteries for the FIGHTER archetypes only (`WeaponMasteryProfile`, Skills.WeaponMasteries.cs), learned @20, applied in `RecomputeDerived` only while the matching weapon is held: Warrior Two-Hand (sword +15% pAtk/+3% crit, blunt +12% pAtk/+10 acc), Rogue Dual (+10% pAtk/+5% crit/+15% crit dmg), Archer Bow (+12% pAtk/+20% crit dmg/+5 acc), Tank (sword/blunt +6% pAtk/+5–10 acc). **Mages (Nuker/Healer) have NO weapon-type mastery** — armor mastery + the flat caster passive carry identity. Nukers now use the SAME `spell_mastery` as healers (replaces base `weapon_mastery`). Both mage passives also carry a **bow penalty: holding a bow halves cast speed** (CastSpeedPct −1 ⇒ ×2 cast time). **Fighter damage tables below do not yet include these — re-derive after tuning.** |
