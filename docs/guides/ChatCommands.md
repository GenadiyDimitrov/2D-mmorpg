...
| `/god` | Toggle **god mode** — you take no damage. Two permanent indicators, not just one chat line: a **red badge** in the top strip (your rank, plus everything you currently have switched on), and a **golden ring** around your own marker. | `/god` |
| `/invis` | Toggle **absolute invisibility** — nobody renders you, nobody (staff included) can target you, and every monster already chasing you loses you. **Nothing in the game ends it**: not attacking, not casting, not an AoE, not the archer's Signal Flare. Type it again to come back. You are still *hittable* by area damage — `/god` is the separate switch for that. The badge turns indigo and reads **INVIS**, and your own marker drops to **0.4 opacity** — that fade is yours alone; to everyone else you are simply not there. | `/invis` |
...
| `/bag <name>` | ⚠ **Server-side only — the Unity client has no window for it.** The server sends the player's inventory; nothing draws it. A WPF-harness survivor. | `/bag Aricel` |
| `/give <name>` | ⚠ **Same — no picker window on this client.** Use the full form below with an id from [ItemIds.md](ItemIds.md). Queued as `BL-56`. | `/give Aricel` |
| `/give <name> <itemId> [sellPrice] [tradable] [timed] ["name"] [enchant] [canStorePrivate] [canStoreAccount] [amount]` | **Spawn a tagged item.** Everything after the id is optional and **positional**; `-` means *no opinion, use the catalog*. `sellPrice`: `-1` unsellable · `0`/`-` catalog price · a number = that price. `timed`: **`1m` is one MINUTE**. `amount` defaults to 1, caps at 10,000 — a stackable arrives as one row, gear as that many rows. Ignores tradability on purpose: staff can hand over anything, quest items included. | `/give Aricel mat_iron - - - - - - - 1000` |
...
| `/giveplat <name> <amount>` | Give or **take** PLATINUM (`BL-257`). Parses exactly like `/givegold` — `k`/`m`/`b`/`t`, `1_000_000`, a negative amount that clamps at zero. ⚠ **It is the ACCOUNT's**, so it lands on every character that account owns, and the confirmation says so. | `/giveplat Aricel 5000` |
