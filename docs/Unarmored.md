...
## How L2 does it
L2's P.Def is the **sum of small per-slot base values**, one per armor slot, and equipping a slot
**replaces that slot's base with the armor piece's P.Def**. Example (L2J, Maestro): chest 31, legs 18,
...
So our naked P.Def is **close to L2's at low level** (68 vs ~80) but, unlike L2, it **grows with level**
(→ 140 by L85) — a naked L85 is a bit tankier than L2 intends. Armor still dominates in both.
...
The clean way to make "naked = fragile" the L2 way is per-slot bases (equipping a slot swaps its base).
...
P.Def/M.Def stay ~flat like L2 (68/20) instead of climbing to 140 — but even that isn't needed.
