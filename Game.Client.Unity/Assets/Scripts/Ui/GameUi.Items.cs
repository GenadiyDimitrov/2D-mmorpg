...
namespace Game.Client
{
    ...
    public partial class GameUi : MonoBehaviour
    {
        ...
        /// <summary>Whether the logged-in account may see staff-only rows on an item card. A static
        /// mirror of <c>Boot.CanUseAdminTools</c>, kept current by <c>LayoutMenuPanel</c>, because the
        /// card builders below are static and have no instance to ask. Defaults to false, so the worst
        /// a stale value can do is hide a staff row — never show one to a player.</summary>
        private static bool StaffTools;
        ...
        private static string ItemStatsText(ItemDef def, InventoryItemDto item)
        {
            ...
            // STAFF ONLY: the def id, so you can read it off the thing in your bag and hand another one
            // out without going to the reference file (playtest-22: *"in each items details in game only
            // for admin to see: a row like the enchant info one with the ID ... So I can use the /give
            // command"*). Sits beside the enchant line by his own description of where it goes. The full
            // grouped list is docs/guides/ItemIds.md, generated by tools/ItemIds.
            if (StaffTools) Line("<color=#7080A0>id  " + def.Id + "</color>");
