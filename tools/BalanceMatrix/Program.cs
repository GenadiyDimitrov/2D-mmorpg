...
// ============================================================================================
//  `--speed` — `BL-238`, MEASURED AGAINST THE BUILT RULE (2026-09-16).
//
//  It began as the two-ordering comparison he asked for (*"can u make me tables with bot
//  formulas"*). He ruled on the page — *"F1 Is Declined"* — and then authored the answer into the
//  CSVs: every Mark row of `healer 4th.csv` and `buffer 4th.csv` now reads *"Decrease movement speed
//  with 10%"*. So this mode stopped being a proposal and became a MEASUREMENT: every number below is
//  `Entity.EffectiveSpeed` off a real character, and the "mark" columns are the game's own answer.
//
//  🔑 WHAT THE RULING IS, in one line: `(base × buffs + flat) × (1 − 0.10)` — the cut lands AFTER the
//  flat shelf, which is the F2 ordering; and the Marks' old `+20% move speed` is GONE, because no CSV
//  row ever authored it. Both halves matter: the +20% was most of why everyone was over 200.
// ============================================================================================
if (args.Length > 0 && args[0] == "--speed")
{
    ...
    Console.WriteLine();
    Console.WriteLine($"=== MOVE SPEED — level {L}, {spQ} gear, FULL NPC shelf — MEASURED ===");
    Console.WriteLine($"    cap {StatCaps.MoveSpeed:0} (* = the row is ON the cap)   ·   sprint +{sprintFlat:0} flat");
    Console.WriteLine();

    // ---- every move-speed source, itemised once --------------------------------------------------
    {
        ...
        float itemFlat = 0f, itemPct = 0f, penalty = 0f;
        foreach (var b in probe.Buffs)
        {
            float f = b.Has(SkillEffect.BuffMoveSpeed) ? b.Flat(SkillEffect.BuffMoveSpeed) : 0f;
            float p = b.Has(SkillEffect.BuffMoveSpeed) ? b.Percent(SkillEffect.BuffMoveSpeed) : 0f;
            if (f == 0f && p == 0f && b.MoveSpeedPenaltyPct == 0f) continue;
            itemFlat += f; itemPct += p; penalty += b.MoveSpeedPenaltyPct;
            string note = b.MoveSpeedPenaltyPct > 0f ? $"   (−{b.MoveSpeedPenaltyPct:P0} AFTER the flats)" : "";
            Console.WriteLine($"  {b.Name,-34} {f,6:+0;-0;0} {p,8:+0%;-0%;-}{note}");
        }
        Console.WriteLine($"  {"= the shelf, summed",-34} {itemFlat,6:+0;-0;0} {itemPct,8:+0%;-0%;-}");
        Console.WriteLine($"  (his \"+69\" is {itemFlat:0} of BUFF flat plus the {bare - tableBase:0} gear and passives fold");
        Console.WriteLine($"   into the BASE. The Mark now SUBTRACTS {penalty:P0}, after the flats — his F2 ordering.)");
        Console.WriteLine();
    }

    static string SpCap(float v) => v >= StatCaps.MoveSpeed - 0.5f ? $"{v:0}*" : $"{v:0}";

    // The shelf hands out ONE Mark. These two build the same character with and without it, so the
    // "no mark" column is a real character rather than arithmetic.
    void StripMark(Entity e)
    {
        if (spMarkKey.Length == 0) return;
        e.Buffs.RemoveAll(b => b.Key == spMarkKey);
        e.RecomputeDerived();
    }
    void AddSprint(Entity e)
    {
        if (SkillCatalog.Get(SkillCatalog.BuffSprint2) is not { } sp) return;
        e.Buffs.Add(new Game.Server.Simulation.BuffInstance
        {
            Effect = sp.Effect, Magnitudes = sp.MagnitudesAt(1) ?? Array.Empty<EffectMagnitude>(),
            TicksRemaining = int.MaxValue, Name = sp.Name, Key = sp.BuffKey, Level = 1,
        });
        e.RecomputeDerived();
    }

    Console.WriteLine($"  {"race",-6} {"role",-8} {"raw",4} {"base",5} {"BARE",6} {"no mark",8} {"MARK",6}"
                    + $" {"+sprint",8} {"mark+spr",9}   (all read off Entity.EffectiveSpeed)");
    var gaps = new List<(string Race, string Role, float Mark)>();
    foreach (var race in spRaces)
        foreach (var (label, pick) in spRoles)
        {
            var (cls, disc) = pick(race);
            Entity Fresh(bool shelf, bool mark, bool sprint)
            {
                var e = Who(race, cls, disc, L, spQ);
                if (shelf) ApplyNpcBuffs(e, fullShelf: true);
                if (shelf && !mark) StripMark(e);
                if (sprint) AddSprint(e);
                return e;
            }
            var bare = Fresh(false, false, false);
            float withMark = Fresh(true, true, false).EffectiveSpeed;
            Console.WriteLine($"  {race,-6} {label,-8} {SpeedTable.BaseRunSpeed(race, cls),4:0} {bare.RunSpeed,5:0}"
                + $" {bare.EffectiveSpeed,6:0} {SpCap(Fresh(true, false, false).EffectiveSpeed),8}"
                + $" {SpCap(withMark),6} {SpCap(Fresh(true, false, true).EffectiveSpeed),8}"
                + $" {SpCap(Fresh(true, true, true).EffectiveSpeed),9}");
            gaps.Add((race.ToString(), label, withMark));
        }

    Console.WriteLine();
    Console.WriteLine("  --- THE BAND: how far the rogue sits above the mage of his own race, Marked, no sprint ---");
    foreach (var race in spRaces)
    {
        float m = gaps.First(g => g.Race == race.ToString() && g.Role == "mage").Mark;
        float r = gaps.First(g => g.Race == race.ToString() && g.Role == "rogue").Mark;
        Console.WriteLine($"  {race,-6} {r - m,6:0.0}");
    }
    Console.WriteLine();
    Console.WriteLine("  ⚠ THE CUT DOES NOT RESERVE THE BAND FOR ANYONE, and no ordering could: it scales the same");
    Console.WriteLine("    base difference by the same factor on every row, so the rogue's lead shrinks with");
    Console.WriteLine("    everyone else's speed. What closed the band is the FLAT shelf, identical on all nine");
    Console.WriteLine("    rows and therefore worth proportionally more to the slowest character. See §5-6 of");
    Console.WriteLine("    docs/balance/MoveSpeedOrderings.md — that half is measured, unbuilt and unruled.");
    return;
}
...
// =====================================================================================================
//  §M (M1-M12, the old material ladder's faucet and recipe costs) was DELETED in 0.205.0 (`BL-273` part 3):
//  it measured the Common…Mythic mat rungs, which no longer exist. `--craft-cost` is the crafting
//  measurement now. BandTtk survives because --craft-cost and --favor-kph run their clock on it.
// =====================================================================================================
...
// =====================================================================================================
//  --craft-cost (`BL-282`): what ONE crafted item costs in KILLS and FARM HOURS under the 2026-09-23
//  rework (`docs/design/Rework-2026-09-23.md` §2.2-2.4): Common/Mythic collapse, recipe % x the mat curve
//  30/50/70/100, the recipe consumed on a fail, the Nightsilver ladder (10:1), essence by grade, 20 heads
//  per 100% recipe, the Volcanic Bar at T76/T80, and the crafter bonus at T76/T80 only.
//
//  🔑 NONE OF THE REWORK IS BUILT. So everything the code does not have yet is ONE input block (C0) at the
//  top of the function, each number tagged with where it came from: NOTE = his 2026-09-23 note, RULED =
//  a later answer, PLACEHOLDER = mine, there only so the table has a number to print. What the code DOES
//  have is measured: the kill clock (M1's calibration) and the gear-drop rate that becomes the Common
//  faucet. Change an input, re-run, read the hours; that is the whole purpose (the entry: "set the chances
//  against a target in hours instead of by hand").
//
//  ⚠ Ingredient hours ADD. Per-mob drop tables (`BL-274`) mean wood, metal, heads and Nightsilver come
//  from different creatures, so the honest clock is the sum. (M12b took the MAX because every mob dropped
...
{
    // ------------------------------------------------------------------------------------------------
    //  C0: INPUTS. Index 0-4 = T40 / T52 / T61 / T76 / T80.
    // ------------------------------------------------------------------------------------------------
    string[] tier = { "T40", "T52", "T61", "T76", "T80" };
    string[] grade = { "D", "C", "B", "A", "S" };
    int[] farmL = { 45, 56, 68, 78, 85 };             // PLACEHOLDER: mid-band, where that tier's crafter farms
    // 🔑 BUILT in 0.205.0 (`BL-273` part 3): the 2H recipe is READ from the catalog (`craft_sword2h_tNN`), so
    // this tool prices exactly what the game charges. The numbers are the note's + the rulings: wood AND iron
    // 400…2000 each, alloy 10…50 (1 alloy = 20 gems + 20 iron), Volcanic Bars 40 / 70 at T76 / T80 (1 bar = 20
    // ash + 20 stone), Nightsilver 300 normal … 10 Legendary (rung = the tier index), 20 parts, essence 400…2000.
    // 1H and the other slots are their own authored cells (Recipes.cs); C4 still prices them at the guide shares.
    Recipe R2H(int t) => RecipeCatalog.Get($"craft_sword2h_t{Crafting.GearTiers[t]}")!;
    int Qty2H(int t, string id) => R2H(t).Inputs.FirstOrDefault(i => i.ItemId == id)?.Qty ?? 0;
    int[] bulk = Enumerable.Range(0, 5).Select(t => Qty2H(t, Crafting.MaterialId(MaterialType.Wood))).ToArray();
    int[] alloy = Enumerable.Range(0, 5).Select(t => Qty2H(t, Crafting.AlloyId)).ToArray();
    int[] bars = Enumerable.Range(0, 5).Select(t => Qty2H(t, ItemCatalog.VolcanicBar)).ToArray();
    int[] nsQty = Enumerable.Range(0, 5).Select(t => Qty2H(t, Crafting.NightsilverId(t))).ToArray();
    int Heads = Qty2H(0, Crafting.PartId("sword2h", 40));
    int[] essence = Enumerable.Range(0, 5).Select(t => Qty2H(t, Crafting.EssenceIds[t])).ToArray();
    ...
    // ------------------------------------------------------------------------------------------------
    var ttkCache = new Dictionary<int, float>();
    float Ttk(int L) => ttkCache.TryGetValue(L, out var v) ? v : ttkCache[L] = BandTtk(L);
    double anchorKph = CoinObserved / Math.Max(1, PerKill(PlaytestLevel).Coin) / 14.5;
    double overhead = Math.Max(0, 3600.0 / Math.Max(1, anchorKph) - Ttk(PlaytestLevel));
    var eliteZones = WorldMap.SpawnZones.Where(z => z.Rank == MobRank.Elite).ToArray();
    double eliteCeil = eliteZones.Length == 0 ? 0
    ...
    // MEASURED (0.200.0): the essence one kill's COMMON gear drops break into, read off the AUTHORED break table
    // (Crafting.BreakYield, `BL-273`), counting only this tier's grade. A Mythic drop is assumed kept, not broken.
    var essCache = new Dictionary<(int, int), double>();
    double BrokenEssencePerKill(int L, int grade)
    {
    ...
    {
        double hN = perKillN > 0 ? 1 / (perKillN * KphN(L)) : double.PositiveInfinity;
        double hE = perKillE > 0 ? 1 / (perKillE * KphE(L)) : double.PositiveInfinity;
        return hN <= hE ? (hN, hN * KphN(L)) : (hE, hE * KphE(L));
    }
    static (double H, double K) Mul((double H, double K) u, double n) => (u.H * n, u.K * n);
    static (double H, double K) Add(params (double H, double K)[] us) => (us.Sum(u => u.H), us.Sum(u => u.K));

    (double H, double K) BaseMat(int L) => Unit(L, BaseMatPerKill(L), BaseMatPerKill(L) * BaseEliteMul);
    (double H, double K) Alloy(int L)
    {
    ...
    (double H, double K) Nightsilver(int L, int rung)
    {
        ...
        double e = rung == 0 ? NsNormalPerKill(L) * EliteMul : L >= nsGateElite[rung] ? NsHigherPerKill * EliteMul : 0;
        var direct = Unit(L, n, e);
        if (rung == 0) return direct;
    ...
    static string CH(double h) =>
        double.IsInfinity(h) || double.IsNaN(h) ? "never"
        : h >= 24 ? $"{h:N0}h ({h / 12:0.#}d)"
        : h >= 1 ? $"{h:0.0}h"
        : $"{h * 60:0.#}m";
    static string CS(double h) =>                   // compact, for the wide C1/C4 grids
        double.IsInfinity(h) || double.IsNaN(h) ? "never" : h >= 1000 ? $"{h / 1000:0.#}kh" : h >= 1 ? $"{h:0.#}h" : $"{h * 60:0.#}m";
    static string CK(double k) => double.IsInfinity(k) || double.IsNaN(k) ? "never" : $"{k:N0}";

    Console.WriteLine("#####################################################################################");
    Console.WriteLine("###  BL-282 --craft-cost: ONE crafted item in kills and farm hours (2026-09-23)    ###");
    Console.WriteLine("#####################################################################################");
    Console.WriteLine("  A 'd' is his 12-hour FARM DAY (auto+offline), as in M8 — not 24 h.");
    Console.WriteLine();
    ...
    (string Name, (double H, double K) Cost, bool Scales)[] Parts(int t)
    {
        int L = farmL[t];
        return new[]
        {
    ...
    Console.WriteLine("=== C1: WHERE THE HOURS GO — one 2H weapon's mats at 100%, per ingredient (farm hours) ===");
    Console.WriteLine($"{"tier",4} {"wood+metal",14} {"alloy",14} {"bars",14} {"heads",14} {"essence",14} {"Nightsilver",14} | {"TOTAL",14}  owner");
    for (int t = 0; t < 5; t++)
    {
        var p = Parts(t);
        double total = p.Sum(x => x.Cost.H);
        var top = p.OrderByDescending(x => x.Cost.H).First();
        Console.WriteLine($"{tier[t],4} " + string.Concat(p.Select(x => $"{CS(x.Cost.H),14} ")) + $"| {CH(total),16}  "
            + $"{top.Name} {top.Cost.H / total:P0}");
    }
    ...
    for (int t = 0; t < 5; t++)
    {
        Console.WriteLine($"{tier[t],4} " + string.Concat(essBySlot[t].Select(e => $"{e,9:N0}"))
            + $" | {CS(Essence(t).H * essBySlot[t][0]),9} {(t < 3 ? $"{(double)essBySlot[t][0] / commonBreak[t]:0.#}" : "-"),10} "
    ...
    // ------------------------------------------------------------------------------------------------
    //  C5b: THE GENERIC-RECIPE TABLE — BUILT 0.204.0 (step 9b + the crafter-points model), read off RecipeCatalog.
    //  Every Scribe/Apothecary recipe: its batch, gates, learn price and BatchValue (what the batch costs to BUY),
    //  and what one batch costs the crafter at three type levels: L0 (x0.90), its own gate, and L10 (x0.55). The
    //  cost 'x' is gold + the fixed inputs at their vendor SELL price (Recipe.GoldAt); 'y' is the batch sold to a
    //  vendor. His rule: crafting then vendoring is 0 or a small loss, so y <= x at every level.
    // ------------------------------------------------------------------------------------------------
    double SellOf(string id) => ItemCatalog.Get(id) is { } sd ? ItemCatalog.SellPrice(sd) : 0;
    Console.WriteLine("=== C5b: THE GENERIC-RECIPE TABLE — BUILT 0.204.0: one BATCH, read off RecipeCatalog ===");
    Console.WriteLine($"{"output",-24} {"type",-10} {"batch",5} {"chr",3} {"gate",4} {"learn",9} | {"batch value",11} | "
        + $"{"gold L0",9} {"gold gate",9} {"gold L10",9} | {"x/buy L0",8} {"@gate",6} {"L10",5} | {"sell y",9} {"y<=x",4} | inputs");
    foreach (var r in RecipeCatalog.All.Where(r => r.BatchValue > 0).OrderBy(r => r.Type).ThenBy(r => r.UnlockLevel).ThenBy(r => r.LearnLevel).ThenBy(r => r.OutputId))
    {
        double inputs = r.Inputs.Sum(i => i.Qty * SellOf(i.ItemId));
        double X(int lvl) => r.GoldAt(lvl) + inputs;
        double y = r.OutputQty * SellOf(r.OutputId);
        bool ok = new[] { 0, r.UnlockLevel, 10 }.All(l => y <= X(l));
        Console.WriteLine($"{r.OutputId,-24} {r.Type,-10} {r.OutputQty,5} {r.LearnLevel,3} {"L" + r.UnlockLevel,4} {r.LearnPrice,9:N0} | {r.BatchValue,11:N0} | "
            + $"{r.GoldAt(0),9:N0} {r.GoldAt(r.UnlockLevel),9:N0} {r.GoldAt(10),9:N0} | {X(0) / r.BatchValue,8:P0} {X(r.UnlockLevel) / r.BatchValue,6:P0} {X(10) / r.BatchValue,5:P0} | "
            + $"{y,9:N0} {(ok ? "ok" : "FAIL"),4} | " + string.Join("+", r.Inputs.Select(i => $"{i.Qty} {i.ItemId.Replace("mat_", "").Replace("_common", "")}")));
    }
    Console.WriteLine("  'batch value' = the batch at its buy price (his base prices); x/buy = gold + inputs (at sell) over it.");
    Console.WriteLine("  Commons sit at L0 for everyone, so their '@gate' column is the L0 price; a gate above the recipe's own");
    Console.WriteLine("  level is unreachable, so only the L0 column reads for a non-Scribe/non-Apothecary crafter's commons.");
    Console.WriteLine("  Enchant + attribute scrolls are NEVER craftable (ruled 2026-09-24); C5c is retired.");
    Console.WriteLine();
}
...
static void ApplyNpcBuffs(Entity e, bool fullShelf = false)
{
    ...
    void Add(SkillDef def, int level)
    {
        ...
        e.Buffs.Add(new Game.Server.Simulation.BuffInstance
        {
            ...
            MoveSpeedPenaltyPct = def.MoveSpeedPenaltyPct,   // `BL-238` - the SEVENTH field channel
...
static void SwapToHarmonyMark(Entity e)
{
    ...
    e.Buffs.Add(new Game.Server.Simulation.BuffInstance
    {
        ...
        MoveSpeedPenaltyPct = mark.MoveSpeedPenaltyPct,
